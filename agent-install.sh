...
fetch_agent_install_config() {
  ...
  local http_code
  local curl_meta
  curl_meta="$(curl -sSL -w "\n%{http_code}\n%{url_effective}" "${config_url}" -o "${INSTALL_CONFIG_PATH}" 2>/dev/null || printf "\n000\n")"
  http_code="$(printf '%s\n' "${curl_meta}" | tail -n 2 | head -n 1)"
  local effective_url
  ...
  if [ "${http_code}" != "200" ]; then
    ...
    if [ -f "${INSTALL_CONFIG_PATH}" ]; then
      err_msg="$(head -n 2 "${INSTALL_CONFIG_PATH}" 2>/dev/null || true)"
    ...
    echo "" >&2
    echo "=================================================================" >&2
    echo " [错误] 获取 Agent 安装配置失败 (HTTP ${http_code})" >&2
    if [ -n "${err_msg}" ]; then
      echo " 服务器返回: ${err_msg}" >&2
    fi
    echo "-----------------------------------------------------------------" >&2
    if [ "${http_code}" = "403" ]; then
      echo " 【原因排查】" >&2
      echo " 当前命令中的 Agent 安装 Token 已失效或已被使用过。" >&2
      echo " 说明：Agent 安装 Token 为一次性安全令牌，且仅在 30 分钟内有效。" >&2
      echo "" >&2
      echo " 【解决步骤】" >&2
      echo " 1. 请回到面板后台 -> 打开当前宿主机的管理详情页；" >&2
      echo " 2. 点击【生成安装命令】（或【重装Agent】）按钮刷新 Token；" >&2
      echo " 3. 重新复制最新生成的完整命令，再次在本机粘贴执行即可。" >&2
      if [ -f "${CONFIG_FILE}" ]; then
        echo "" >&2
        echo " 提示：检测到本机已有配置文件 (${CONFIG_FILE})。" >&2
        echo " 若此前已安装成功且仅需更新 Agent，可直接执行:" >&2
        echo "   systemctl restart ${SERVICE_NAME}" >&2
      fi
    elif [ "${http_code}" = "000" ]; then
      echo " 无法连接到面板地址: ${PANEL_BASE_URL}" >&2
      echo " 请检查母鸡网络连通性、DNS 解析或面板域名是否可访问。" >&2
    else
      echo " 请求面板异常 (HTTP ${http_code})，请检查面板后台服务状态。" >&2
    fi
    echo "=================================================================" >&2
    echo "" >&2
    fail "Agent install token invalid or expired (HTTP ${http_code})"
  fi
  ...
  # 若面板存在 301/302 重定向（如从根域名自动跳到 www 域名），自动更新为规范面板地址
  if [[ "${effective_url}" =~ ^(https?://[^/]+) ]]; then
    local canonical_panel="${BASH_REMATCH[1]}"
    if [ -n "${canonical_panel}" ] && [ "${canonical_panel}" != "${PANEL_BASE_URL}" ]; then
      echo "info: panel URL redirected from ${PANEL_BASE_URL} to ${canonical_panel}"
      PANEL_BASE_URL="${canonical_panel}"
      INCUDAL_PANEL_URL="${canonical_panel}"
